...
def _build_dynamic_v2_layout(
    ...
    if questions_count < 1:
        raise ValueError("questions_count deve ser >= 1")
    if questions_count > 40:
        raise ValueError("V2 suporta ate 40 questoes objetivas por folha.")
    ...
    column_count = 1 if questions_count <= max_rows_per_column else 2
    rows_per_column = (
        questions_count
        if column_count == 1
        else min(max_rows_per_column, (questions_count + 1) // 2)
    )
    ...
    machine_height_pdf = (
        machine_top_padding
        + column_header_height_formula
        + (rows_per_column * row_height_formula)
        + machine_bottom_padding
        + machine_internal_margin
        + safety_buffer
    )
    ...
    row_width_pdf = 122.0
    if column_count == 1:
        column_x0_values = [(cls.CARD_WIDTH_PDF - row_width_pdf) / 2.0]
    else:
        side_margin_pdf = 7.0
        gap_pdf = cls.CARD_WIDTH_PDF - (2.0 * row_width_pdf) - (2.0 * side_margin_pdf)
        column_x0_values = [
            side_margin_pdf,
            side_margin_pdf + row_width_pdf + max(4.0, gap_pdf),
        ]

    bubble_centers_by_column_pdf = [
        [
            column_x0 + 39.0 + cls.BUBBLE_CENTER_X_OFFSET_PDF,
            column_x0 + 55.0 + cls.BUBBLE_CENTER_X_OFFSET_PDF,
            column_x0 + 71.0 + cls.BUBBLE_CENTER_X_OFFSET_PDF,
        ...
        for column_x0 in column_x0_values
    ]
    bubble_centers_by_column = [
        [x * scale for x in column_centers]
        for column_centers in bubble_centers_by_column_pdf
    ]
    ...
    content_height_pdf = (
        header_visual_pdf
        + (rows_per_column * row_pitch_pdf)
        + footer_visual_pdf
    )
    available_height_pdf = max(0.0, machine_height_pdf - (2.0 * outer_vertical_margin_pdf))
    free_space_pdf = max(0.0, available_height_pdf - content_height_pdf)
    top_inside_centered_block_pdf = outer_vertical_margin_pdf + (free_space_pdf / 2.0)
    first_row_center_y_pdf = (
        top_inside_centered_block_pdf
        + header_visual_pdf
        + (11.0 / 2.0)
    ...
    bubbles: List[BubbleSpec] = []
    bubble_map: Dict[int, Dict[str, BubbleSpec]] = {}
    question_centers_y: List[float] = []
    for q_idx in range(questions_count):
        question_number = q_idx + 1
        column_index = 0 if column_count == 1 else q_idx // rows_per_column
        row_index = q_idx if column_count == 1 else q_idx % rows_per_column
        cy = base_row_centers_y[row_index]
        question_centers_y.append(cy)
        bubble_centers_x = bubble_centers_by_column[column_index]
        bubble_map[question_number] = {}
        for choice, cx in zip(choices, bubble_centers_x):
            spec = BubbleSpec(
                question=question_number,
                option=choice,
                x=float(cx),
    ...
    return BubbleSheetLayout(
        ...
        bubble_radius=bubble_radius,
        bubble_centers_x=[
            x for column_centers in bubble_centers_by_column for x in column_centers
        ],
        row_centers_y=question_centers_y,
        bubbles=bubbles,
        bubble_map=bubble_map,
        anchor_points=anchor_points,
        expected_machine_ratio=machine_width / float(machine_height),
        debug={
            ...
            "first_row_center_y_pdf": first_row_center_y_pdf,
            "row_pitch_pdf": row_pitch_pdf,
            "bubble_centers_by_column_pdf": bubble_centers_by_column_pdf,
            "row_center_y_offset_pdf": cls.ROW_CENTER_Y_OFFSET_PDF,
            "bubble_center_x_offset_pdf": cls.BUBBLE_CENTER_X_OFFSET_PDF,
        },
    )
...
AcademyHubLayoutAdapter.MAX_QUESTIONS = 40
AcademyHubLayoutAdapter.MAX_ROWS_PER_COLUMN = 20
AcademyHubLayoutAdapter.build = classmethod(_build_dynamic_v2_layout)
